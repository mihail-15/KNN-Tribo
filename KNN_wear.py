...
# Code lines for Al–Al2O3 composites infiltrated with B83 (B)
# Read each .xlsx file containing the data for each dataset of the material B83 with 40 N load
file_B1 = pd.read_excel("composite_1.xlsx")
file_B2 = pd.read_excel("composite_2.xlsx")
file_B3 = pd.read_excel("composite_3.xlsx")
...
cof_B1 = friction_B1 / 50 # COF for B1
cof_B2 = friction_B2 / 50 # COF for B2
cof_B3 = friction_B3 / 50 # COF for B3
...
# load the data from the ‘xlsx’ file (in our case “COF_B.xlsx”)
...
# define a parameter grid with the hyperparameters and their ranges for kNN
param_grid = {
...
knn = KNeighborsRegressor() # use default parameters

# create a grid search object with 5-fold cross-validation
gs = GridSearchCV(knn, param_grid, scoring='neg_mean_squared_error', cv=5, n_jobs=-1)
...
with open('B_performance_metrics.txt','w') as f:
    ...
    f.write('MAPE: {:.4f}\n'.format(mean_absolute_percentage_error(y_val,y_val_pred)))
    f.write('MSLE: {:.4f}\n'.format(mean_squared_log_error(y_val,y_val_pred)))
    f.close()
...
# Add a title
plt.title("AMMCs infiltrated with B83", fontsize='18', fontweight='bold')
...
plt.title('Histogram of COF for AMMCs infiltrated with B83', fontsize='16', fontweight='bold')
plt.show()
